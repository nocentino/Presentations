...
ssh aen@c1-node1 'sudo mkdir -p /data/mssql-data'


cd ~/talks/Talks/Kubernetes/GroupBy/demos
...
#Change out the version of SQL Server from CU11->CU12, could use YAML or kubectl edit too
...
#ReplicaSet goes from 1->0 on CU12 and from 0-1-> on CU11. Resued the old replicaset, see pod template hash
kubectl describe deployment mssql-deployment
