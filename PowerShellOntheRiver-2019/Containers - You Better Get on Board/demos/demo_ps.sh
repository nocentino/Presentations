...
docker pull mcr.microsoft.com/powershell:6.2.2-ubuntu-18.04
docker pull mcr.microsoft.com/powershell:preview #7.0.0-preview.2
...
$PSVersionTable
Get-Process
"Hello PSOTR Attendees!!!" | Out-File -path ~/greetings.txt
Get-Childitem -path ~/
exit

#When we exit, our process terminates and thus our container is no longer running...why?
docker ps
...
#PowerShell Core 6.2.2, using the latest tag...and passing in a cmdlet into the container.
docker run mcr.microsoft.com/powershell:latest pwsh -c "&{Get-Process}"

#PowerShell Core 6.2.2, specifying a specific tag/PowerShell Core verison and platform.
docker run mcr.microsoft.com/powershell:6.2.2-ubuntu-18.04 pwsh -c "&{Get-Process}"

#PowerShell Core 7.0.0-preview.2, getting crazy and running preview. 
