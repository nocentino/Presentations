...
#Check out the servername and the version in the container, 2019 RTM-GDR
...
#Change out the version of SQL Server from GDR->CU1, could use YAML or kubectl edit too
kubectl set image deployment mssql-deployment mssql=mcr.microsoft.com/mssql/server:2019-CU1-ubuntu-16.04 --record
...
#ReplicaSet goes from 1->0 on CU1 and from 0->1 on GDR1. Resued the old replicaset, see pod template hash
kubectl describe deployment mssql-deployment
