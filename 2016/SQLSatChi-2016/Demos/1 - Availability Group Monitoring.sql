...
/*
	...
	Demo 3 - Examine send queue and redo queue
		...
		2. start 25 user workload, examine log_send_rate, log_redo_rate and also redo_queue_size
...
/*
	...
	Setup -	Offline SQL14-C while workload running....
			Start 25 user workload, after 30 seconds...

		1. On first query, Check out the state of SQL14-C.

		2. On second query, but do not query SQL14-C...it's offline. 	
			Look at send queue on primary....NULL! Ugh!!!  Why? Because the secondar is the one that reports the status!
			Check out the redo queue on SQL14-B, should see a small amount of queuing

		Online SQL14-C after workload completes
		1. Rerun queries, when SQL14-C goes to synchronizing, send_queue will be reported. redo_queue will be updates.
