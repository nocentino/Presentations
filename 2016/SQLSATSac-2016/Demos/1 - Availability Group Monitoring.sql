...
/*
	...
	Demo 3 - Examine send queue and redo queue
		...
		Use SQLServer:Database Replica - Log Bytes Compressed/Sec for how much log is being compressed. This is per replica. 
		Use SQLServer:Availability Replica - Bytes Send to Replica/sec for actual compressed send rates. A counter for each replica.
		Use Network Adapter:Interface Name - Bytes Sent/Sec to see what's being transmitted on the network

		On the secondary
		Use SQLServer:Database Replica - Log Bytes Deompressed/Sec for how much log is being decompressed on this replica. 
		Use SQLServer:Database Replica - Log Bytes Received/sec (log send rate) for actual decompressed send rates 
		Use SQLServer:Database Replica - Redone Bytes/sec (log redo rate)
...
SET @log_flushes1 = (SELECT cntr_value FROM sys.dm_os_performance_counters	
				WHERE [object_name] = 'SQLServer:Databases' and instance_name = 'TestAG1' and counter_name = 'Log Bytes Flushed/sec')
