#Remove any existing data in this location, we'll be putting new data there in this demo.
ssh aen@c1-master1
ssh aen@c1-node1 
sudo rm -rf /data/mssql-data
sudo mkdir -p /data/mssql-data
sudo ls /data/mssql-data
exit

PS1='\u@\h:\W\$ '
cd ~/content/presentations/PowerShellOntheRiver-2019/Kubernetes\ -\ Architecture\ Fundamentals/demos
...
#Check out the servername and the version in the container, 2017 CU11
...
#Change out the version of SQL Server from CU12->CU15, could use YAML or kubectl edit too
...
#ReplicaSet goes from 1->0 on CU12 and from 0-1-> on CU15. Resued the old replicaset, see pod template hash
kubectl describe deployment mssql-deployment
