...
docker pull mcr.microsoft.com/powershell:latest               #Currently 7.1.5
docker pull mcr.microsoft.com/powershell:7.1.5-ubuntu-18.04
docker pull mcr.microsoft.com/powershell:preview-ubuntu-18.04 #7.2.0-preview
...
$PSVersionTable
Get-Process
"Hello RTPSUG Attendees" | Out-File -path ~/greetings.txt
Get-Childitem -path ~/
exit
...
docker run mcr.microsoft.com/powershell:preview-ubuntu-18.04 pwsh -c "&{Get-Host}"
...
docker run                      \
    ...
    --volume PSScripts:/scripts \
      mcr.microsoft.com/powershell:7.1.5-ubuntu-18.04
...
#Or maybe even pull from a code repository
...
docker run                       \
    --rm                         \
    --volume PSScripts:/scripts  \
      mcr.microsoft.com/powershell:7.1.5-ubuntu-18.04 pwsh -F /scripts/Get-Containers.ps1
...
az vm list-sizes --location eastus -o tsv  | head


open https://github.com/Azure/CloudShell/blob/master/build.ps1
