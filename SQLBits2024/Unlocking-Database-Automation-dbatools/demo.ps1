...
docker run `
    ...
    -d mcr.microsoft.com/mssql/server:2022-CU11-ubuntu-22.04 
...
#   - Restoring a subset of backups, and in this example we're restoring from file
...
Backup-DbaDatabase -SqlInstance $SqlInstance -CompressBackup -AzureBaseUrl 'https://testdbaen.blob.core.windows.net/testdb' -Type Log  -WhatIf
...
docker stop sql1
docker rm sql1
docker volume ls | grep sqldata1_0


# Create a container with multiple data files spread across multiple volumes
docker run `
    --name "sql1" `
    --hostname "sql1" `
    -e "ACCEPT_EULA=Y" `
...
# Let's stop and delete the current SQL container, because we want to reconfigure it but leave behind the volumes and its data
docker stop sql1 
docker rm sql1
...
# We're then re-attaching the existing voumes which already have data in them from the previous demo
docker run `
    --name "sql1" `
    --hostname "sql1" `
    --net agnetwork `
...
# Create a second SQL instance with the same configuration as above.
docker run `
    --name "sql2" `
    --hostname "sql2" `
    --net agnetwork `
...
# Build connections to both SQL instances to reuse in this demo
...
# Let's start off with sorting authentication, since we're using linux containers...I'm gonna opt for certificates. 
# Also good to consider when using AD too. Why? 
...
# Restore the certificate on the soon to be secondary replica
...
# Now, let's create the AG, lots 'o parameters. 
# This creates a clusterless, manual failover AG using the certificate we just created to authenticate the database mirroring endpoints
...
# Check the status of the newly added AG database
...
# Try a co-pilot demo here....
# give me a dbatools to get the cost threshold for parallelism
